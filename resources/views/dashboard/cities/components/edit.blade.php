<div class="modal fade modal-secondary text-start" id="EditCity{{ $single->id }}" tabindex="-1" aria-labelledby="myModalLabel1660" aria-hidden="true">
                ...
                <section id="basic-vertical-layouts">
                    <div class="row">
                        <div class="col-md-12 col-12">
                            <form id="updateCityForm{{ $single->id }}" class="form form-vertical" method="POST">
                                @csrf @method('PUT')
                                <input id="countryId{{ $single->id }}" type="hidden" name="country_id" value=""/>
                                <div class="row">
                                    {{-- edit en name --}}
                                    ...
                                    {{-- edit country --}}
                                    <div class="col-12 mb-1">
                                        <label class="form-label" for="country">@lang('translate.country') <span class="text-danger">*</span></label>
                                        <div class="input-group input-group-merge">
                                            ...
                                            <select id="editSelectedCountry{{ $single->id }}" class="form-control editSelectedCountry">
                                                ...
                                                @foreach ($countries as $country)
                                                    <option value="{{ $country->id }}" {{ $single->country_id == $country->id ? 'selected' : '' }}>{{ $country->$nameOnLang }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
