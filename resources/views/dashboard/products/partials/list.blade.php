                ...
                <tbody>
                    @foreach ($data as $single)
                        <tr>
                            ...
                            {{-- avatar --}}
                            <td>
                                @include('dashboard.partials.image-modal', ['id' => $single->id, 'name' => $single->$nameOnLang, 'image' => asset($single->avatar ? 'storage/images/products/' . $single->avatar : 'storage/images/global/default.jpg')])
                            </td>
                            ...
                            @if (auth('vendor')->check() || (auth('admin')->check() && auth('admin')->user()->is_primary))
                                <td>
                                    <form class="p-0 m-0" action="{{ route('toggleActive', ['table' => 'product', 'id' => $single->id]) }}" method="post">
                                        @csrf
                                        <label class="switch">
