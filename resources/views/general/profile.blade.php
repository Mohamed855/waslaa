...
@section('content')
    <section id="basic-horizontal-layouts">
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <img src="{{ asset($authUser['avatar'] ? 'storage/images/' . $guard . 's/' . $authUser['avatar'] : 'storage/images/global/profile.jpg') }}" class="card-img-top" alt="profile image">
                    <div class="card-body">
                        <div class="row">
                            @if($authUser['avatar'] == null)
                                <form action="{{ route('avatar.update', ['guard' => $guard, 'id' => auth($guard)->id()]) }}" id="avatar_form"
                            ...
                            @else
                                <div class="row justify-content-between m-auto">
                                    <form action="{{ route('avatar.update', ['guard' => $guard, 'id' => auth($guard)->id()]) }}" id="avatar_form"
                                          method="post" enctype="multipart/form-data" class="d-inline cursor-pointer col-6">
                                        @csrf
                                    ...
                                    <form action="{{ route('avatar.remove', ['guard' => $guard, 'id' => auth($guard)->id()]) }}" method="post"
                                          class="d-inline cursor-pointer col-6">
                                        @csrf
            ...
            <div class="col-md-8">
                <div class="card  p-2">
                    <div class="card-body">
                        ...
                        <p class="card-text">@lang('translate.joinedAt'): {{ date_format($authUser['created_at'], 'd-m-Y') }}</p>
                    </div>
                    <div class="row">
                        <div class="col-md-12 col-12 m-auto">
                            <div class="card">
                                <div class="card-body">
                                    <form method="POST" action="{{ route('profile.update', ['guard' => $guard, 'id' => auth($guard)->id()]) }}"
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <form method="POST" action="{{ route('password.change') }}" class="form form-vertical">
                                        @csrf
                                        <div class="row mb-2">
                                            {{-- new password --}}
                                            <div class="col-md-6 col-sm-12 mb-1">
